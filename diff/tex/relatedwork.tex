...
XFI~\cite{erlingsson2006xfi} is the most representative unconventional PCC work based on SFI, which places a verifier at OS level, instead of a TEE. \revise{Our compiler-based generator is more efficient in providing forward-edge CFI and our runtime enforcement is simpler than inline reference monitor or dynamic binary translation used by traditional SFI}~\cite{zhou2014armlock,tan2017principles}. \DIFdelbegin \DIFdel{Occlum~\mbox{%DIFAUXCMD
\cite{shen2020occlum} }\hspace{0pt}%DIFAUXCMD
is a design of SGX-based library OS, enforcing in-enclave task isolation with  MPX-based multi-domain SFI scheme. Side channels pose serious threats~\mbox{%DIFAUXCMD
\cite{lee2017inferring,wang2017leaky,van2018foreshadow,chen2019sgxpectre}}\hspace{0pt}%DIFAUXCMD
, while existing work assumes side channels as an orthogonal research topic~\mbox{%DIFAUXCMD
\cite{sinha2015moat,subramanyan2017formal,shen2020occlum}}\hspace{0pt}%DIFAUXCMD
. %DIF < This leaves a challenge to integrate SGX secure computing framework with SGX side defense. 
Our framework is designed with side channels in mind and it can flexibly support integration of instrumentation based sidechannel defenses~\mbox{%DIFAUXCMD
\cite{shinde2016preventing,shih2017t,oleksenko2018varys,sinha2017compiler,chen2018racing}}\hspace{0pt}%DIFAUXCMD
.
%DIF < As the goal of Occlum and others is not to prevent information leakage from untrusted code, none of them employs protections against side channel leakages. 
%DIF < It has a distributed verification design, the TCB introduced by the Library OS is substantial non-negligible.
}\DIFdelend \DIFaddbegin \DIFadd{The advantage of our design compared to other state-of-the-art shielding runtimes (e.g., Occlum) is three-fold. Firstly, DEFLECTION is more general. The memory access check of Occlum relies on hardware (Intel MPX) which is no longer supported, significantly hindering deployment. Secondly, DEFLECTION has a smaller size of TCB. Other than importing Zydis and PyVEX (in Python) to be the disassembler and verifier, we shrank and modified Capstone (in C) to implement our smaller disassembler and verifier. Thirdly, DEFLECTION can mitigate some side/covert channel leaks while others provide no such protection.
}\DIFaddend 
